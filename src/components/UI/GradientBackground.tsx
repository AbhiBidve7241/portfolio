...
vec3 col = vec3(
  0.6 + 0.4 * sin(p.x * 1.5 + time * 0.6),
  0.6 + 0.4 * sin(p.y * 1.3 + time * 0.8),
  0.6 + 0.4 * sin(p.x + p.y + time * 0.4)
);

// color mixing for richer gradients
col = mix(col, vec3(0.8, 0.5, 1.0), 0.3);
col = mix(col, vec3(0.3, 0.8, 1.0), 0.2);
 col *= 1.2;
col = pow(col, vec3(0.9)); // softer glow
  ...
  fragColor = vec4(col, extrusion * 0.25);
}
...
const compileShader = (
  ...
  // ✅ IMPORTANT: use program BEFORE getting uniforms
  ...
  const render = () => {
    if (!iResolutionLocation || !iTimeLocation) return;
    ...
    gl.useProgram(program); // ✅ ensure active
